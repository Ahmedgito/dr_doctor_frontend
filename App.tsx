...
export default function App() {
  ...
  // Auto-scroll to bottom using scrollTop to avoid shifting the whole viewport
  useEffect(() => {
  ...
  return (
    <div className="flex h-full w-full relative overflow-hidden transition-colors duration-500 bg-slate-900">
      
      {/* Background Ambience */}
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-teal-600/20 rounded-full blur-[120px] pointer-events-none animate-pulse-slow"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-blue-600/10 rounded-full blur-[120px] pointer-events-none animate-pulse-slow" style={{animationDelay: '1.5s'}}></div>
      ...
      <div className={`fixed inset-0 z-50 md:hidden transition-all duration-300 ${isSidebarOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
        ...
        />
        <aside className={`absolute left-0 top-0 bottom-0 w-[85%] max-w-[320px] bg-slate-900/95 backdrop-blur-2xl border-r border-white/10 flex flex-col shadow-2xl transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="p-5 flex-shrink-0 flex items-center justify-between border-b border-white/10">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-lg text-white">dr.doctor</span>
                </div>
                <button onClick={() => setIsSidebarOpen(false)} className="p-2 text-slate-400 hover:text-white rounded-lg hover:bg-white/10 transition-colors">
                    <X size={20} />
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                 <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                     <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm font-semibold text-slate-200">Secure & Private</p>
                            <p className="text-[10px] text-slate-400">HIPAA Compliant AI</p>
                        </div>
                    </div>
                </div>
      ...
      <aside className="hidden md:flex flex-col w-80 h-[95vh] my-auto ml-4 rounded-3xl glass-panel shadow-2xl z-20 transition-all duration-300">
        <div className="p-6 pb-2">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="font-bold text-2xl text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-blue-500 tracking-tight">
                        dr.doctor
                    </h1>
                </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 px-4 gap-4">
            <div className="p-4 rounded-2xl bg-gradient-to-br from-white/5 to-white/0 border border-white/5 relative overflow-hidden group">
            ...
            <div className="flex-1 overflow-y-auto px-2 -mx-2">
                ...
                <div className="space-y-1">
                    {recentChats.map((chat) => (
                        <button 
                            key={chat.id}
                            className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-all group text-left border border-transparent hover:border-white/5 hover:translate-x-1"
                        >
                            <MessageSquare size={16} className="text-slate-500 group-hover:text-teal-400 transition-colors" />
                            <div className="flex-1 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col relative h-full z-10 md:p-4 min-w-0">
        ...
        {/* Chat Container */}
        <div className="flex-1 flex flex-col bg-slate-900/60 backdrop-blur-xl md:rounded-3xl shadow-2xl border-white/10 md:border md:border-white/10 relative overflow-hidden">
            
            {/* Mobile Header - Fixed positioning within flex column */}
            <div className="md:hidden flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-white/5 bg-slate-900/80 backdrop-blur-xl z-20">
                 <div className="flex items-center gap-3">
                    <button 
                ...
                <div className="flex items-center gap-1">
                    <button onClick={toggleTheme} className="p-2 text-slate-400 hover:text-white rounded-full hover:bg-white/5">
                         {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
                    </button>
                    <button 
                        onClick={requestLocation}
                        className={`p-2 rounded-full transition-all ${locationStatus === 'granted' ? 'text-teal-400 bg-teal-400/10' : 'text-slate-400 bg-white/5'}`}
                    >
                        <MapPin size={18} />
                    </button>
                </div>
            ...
            <div className="p-4 z-20 flex-shrink-0">
                <div className="max-w-3xl mx-auto w-full">
                            ...
                            <button 
                                ...
                                className={`
                                    ...
                                    ${activeFilters.includes(filter.id as FilterType) 
                                        ? 'bg-teal-500/20 border-teal-500/50 text-teal-300 shadow-[0_0_15px_rgba(20,184,166,0.3)]' 
                    ...
                    <div className="relative group">
                        ...
                        <div className="relative flex items-end bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-2xl shadow-xl transition-all">
                            <textarea
                                value={inputValue}
                                onChange={(e) => setInputValue(e.target.value)}
                                onKeyDown={handleKeyPress}
                                placeholder="Describe your symptoms..."
                                className="w-full bg-transparent border-none focus:ring-0 text-slate-100 placeholder-slate-500 pl-4 md:pl-5 pr-20 md:pr-24 py-4 resize-none max-h-32 text-sm md:text-[15px] leading-relaxed"
                                rows={1}
                            ...
                            <div className="absolute right-2 bottom-2.5 flex items-center gap-1">
                                <button 
                                    className="p-2.5 text-slate-400 hover:text-teal-400 rounded-xl hover:bg-white/5 transition-colors group/mic relative hidden md:block"
                                    title="Record voice"
                                >
                                    <Mic size={20} />
